...
public class ChatMainActivity extends BaseActivity implements OnClickListener,ReceiveInfoListener{
...
//		l2.setOnClickListener(this);
	...
	private void gueryDataBase() {
		...
		if(GroupList!=null&&GroupList.size()>0){
			for(int i=0;i<GroupList.size();i++){
				...
				Bitmap bm=FileUtil.getBitmap(FileUtil.APP_PATH+"/group"+"/"+userId+"/"+group.getGroupId()+".jpg");
				group.setBm(bm);
			}
		}
		...
		if(FriendList!=null&&FriendList.size()>0){
			...
			for(Friend friend :FriendList){
				Bitmap bm=FileUtil.getBitmap(FileUtil.APP_PATH+"/friend"+"/"+userId+"/"+friend.getFriendID()+".jpg");
				friend.setBm(bm);
			}
		}
	...
	void InItView(){
		...
		if(getLocalActivityManager().getActivity("ChatAttentionActivity")!=null){
			...
			ChatAttentionActivity chatAtt=(ChatAttentionActivity) getLocalActivityManager().getActivity("ChatAttentionActivity");
...
//			intent.putParcelableArrayListExtra("list", list);
			view02 = getLocalActivityManager().startActivity("GroupListActivity",
					intent).getDecorView();
		}
		...
		if(getLocalActivityManager().getActivity("FriendListActivity")!=null){
			...
			FriendListActivity friend=(FriendListActivity) getLocalActivityManager().getActivity("FriendListActivity");
			friend.upDate(FriendList);
			if(FriendList.size()>0){
			Log.i(tag,"FriendList.friendID:######"+FriendList.get(0).getFriendID());
			}else{
				Log.i(tag,"大于0");
			}
			...
			intent.putExtra("userId", userId);
			intent.putParcelableArrayListExtra("list", list);
			view01 = getLocalActivityManager().startActivity("FriendListActivity",
					intent).getDecorView();
		}
...
//			}
	...
	Runnable runnable=new Runnable(){
		...
		@Override
		public void run() {
			LogUtil.p(tag, "看群------userID:"+BaseActivity.getSelf().getUserID());
...
//			con.getOfflineMessage(userId);
	...
	public void processMessage(Message msg) {
		...
		if(msg.what==0){//0表示后台返回的是群列表的查询结果。1表示返回的是查询私信列表返回的结果
			...
			for(Friend friend:FriendList){//判断这个人是否已经在列表项
				...
				else if(friend.getFriendID().equals(chatMessage.getFriendID())){
			...
			if(isAdd){
				...
				FriendList.add(new Friend(chatMessage.getFriendID(),chatMessage.getNickName(),FileUtil.getBitmap(FileUtil.APP_PATH+"/friend"+"/"+userId+"/"+chatMessage.getFriendID()+".jpg")));
			}
			BaseActivity.saveMessagesToDB(chatMessage.getSelfID(),chatMessage.getFriendID(),chatMessage.getNickName(),Config.MESSAGE_FROM,chatMessage.getType(),chatMessage.getTime(),chatMessage.getContent());
			if(FriendList.size()>0){
				Log.i(tag,"FriendList.friendID:%%%%%%%%%"+FriendList.get(0).getFriendID());
				}else{
					Log.i(tag,"大于0");
	...
	public void insertGroupList(List<Group> groupList){
		...
		//先将图片存入本地sd卡
		File file=FileUtil.createFile(userId, Integer.parseInt(group.getGroupId()));
		FileUtil.saveBitmap(file, group.getBm());
		//将该群信息插入数据库
		saveToDB(userId,Integer.parseInt(group.getGroupId()),group.getGroupName());
		}	
	}
