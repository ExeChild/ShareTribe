...
public class NewNetWorkSend {
	...
	/*// 发送文本私信消息
	public void sendMessage(String userId, String friendId, String content) {
...
*/
	...
	public boolean sendText(String userID,String friendID,String time,String content){
		boolean result = true;
		Log.i(TAG, "NewNetWorker01---告诉服务器客户端要发送信息");
		buffer = ByteBuffer.allocateDirect(24+userID.getBytes().length+friendID.getBytes().length
				+time.getBytes().length+content.getBytes().length);
		buffer.putInt(Config.MESSAGE_TO);
		buffer.putInt(Config.MESSAGE_TEXT);
		Log.i(TAG, "NewNetWorker01---发送Config.MESSAGE_TO和Config.MESSAGE_TYPE_TXT请求");
		buffer.putInt(userID.getBytes().length);
		buffer.put(userID.getBytes());
	...
	public boolean sendImg(String userID,String friendID,String time,String filePath){
		...
		buffer = ByteBuffer.allocateDirect(28+userID.getBytes().length+friendID.getBytes().length
				+time.getBytes().length+getFileName(filePath).getBytes().length+imgByte.length);
		buffer.putInt(Config.MESSAGE_TO);
		buffer.putInt(Config.MESSAGE_IMG);
		Log.i(TAG, "NewNetWorker01---发送Config.MESSAGE_TO和Config.MESSAGE_IMG请求");
		buffer.putInt(userID.getBytes().length);
		buffer.put(userID.getBytes());
		buffer.putInt(friendID.getBytes().length);
		buffer.put(friendID.getBytes());
	...
	// 新建群//****
	public void sendCreateGroup(String userId,String groupName,String groupDesc,String groupLabel,int groupProperty,String groupHeadImg,String path) throws IOException {
		//String path="sdcard/create_group_headimg.png";
		byte[] bytes=makeFileToByte(path);
		
		try{
		buffer = ByteBuffer.allocate(36 + userId.getBytes().length+groupName.getBytes().length+
				groupDesc.getBytes().length+groupLabel.getBytes().length+groupHeadImg.getBytes().length+bytes.length);
...
//			String userID = WoliaoBaseActivity.getSelf().getUserID();
	...
	public void findCrowd(String condition,String userId) {
		...
		buffer = ByteBuffer.allocate(12 +userId.getBytes().length+ condition.getBytes().length);
...
//		put(condition);
	...
	//请求查看朋友信息
		public void reqPersonINFO(String friendID){
			...
			buffer = ByteBuffer.allocateDirect(12+friendID.getBytes().length+userID.getBytes().length);
		...
		public void reqAttention(String userID,String friendID){
			...
			buffer = ByteBuffer.allocateDirect(12+userID.getBytes().length+friendID.getBytes().length);
